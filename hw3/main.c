...
void* satellite_thread_func(void* arg) {
    ...
    int wait_result = sem_timedwait(&request->request_handled_sem, &request->timeout_deadline);
...
void* engineer_thread_func(void* arg) {
    ...
    while (true) {
        ...
        timespec_add_seconds(&wait_time, 1); // Wait for 1 second at a time

        int sem_status = sem_timedwait(&newRequest, &wait_time);

        if (sem_status == -1 && errno != ETIMEDOUT) {
        ...
        if (should_shutdown && sem_status == -1 && errno == ETIMEDOUT) {
             pthread_mutex_unlock(&engineerMutex);
        ...
        if (!should_shutdown && availableEngineers > 0 && requestQueue != NULL) {
...
int main() {
    ...
    // Signal engineers potentially blocked on sem_wait to check for termination
    // This helps ensure they exit cleanly if they were waiting when the last satellite finished.
    // Note: Engineers also use timed wait, so they will wake up periodically anyway.
    // Posting ensures faster shutdown if they are deeply blocked.
     for (int i = 0; i < NUM_ENGINEERS; i++) {
        sem_post(&newRequest);
    }
